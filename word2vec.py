...
def naiveSoftmaxLossAndGradient(
    ...
    gradOutsideVecs -- the gradient with respect to all the outside word vectors
                    (dJ / dU)
    """
    #guess the outer words in the goal
    guess = softmax(np.dot(centerWordVec, outsideVectors.T))                       
    delta = guess.copy()
    delta = delta[outsideWordIdx] - 1
    
    loss = -np.log(guess)[outsideWordIdx]
    
# We just use OutsideWordVecs because the derivative of the softmax of centerword just leaves us with the outside words
    gradCenterVec = np.dot(delta, outsideVectors) 
    gradOutsideVecs = np.dot(delta[:, np.newaxis], centerWordVec[np.newaxis, :])

# We seek to optimize the loss and we can change Center grad and Context gradient to optimize for it
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    ### YOUR CODE HERE
    #Guess the center word is the goal
